...
export const VersionControls: React.FC<VersionControlsProps> = ({ 
  ...
  const hasActualChanges = React.useMemo(() => {
    if (!schema || !patches) {
      console.log('❌ VersionControls: Missing schema or patches, falling back to isModified:', { hasSchema: !!schema, hasPatches: !!patches, isModified });
      return isModified;
    }
    
    try {
      // Use the imported applySelectedPatches function
      const selectedPatches = patches.filter(p => p.isSelected);
      
      console.log('🔍 VersionControls: Checking for actual changes:', {
        totalPatches: patches.length,
        selectedPatches: selectedPatches.length,
        selectedPatchDescriptions: selectedPatches.map(p => p.description),
        schemaLength: schema.length,
        schemaPreview: schema.substring(0, 100)
      });
      ...
      if (selectedPatches.length === 0) {
        ...
        console.log('📝 VersionControls: No selected patches, checking for non-empty schema:', { hasChanges });
        return hasChanges;
      }
      
      const currentMergedSchema = applySelectedPatches(selectedPatches);
      const currentMergedString = JSON.stringify(currentMergedSchema, null, 2);
      const actualChanges = schema !== currentMergedString;
      
      console.log('📊 VersionControls: Comparing schemas:', {
        schemaLength: schema.length,
        mergedLength: currentMergedString.length,
        hasChanges: actualChanges,
        isModified,
        schemaPreview: schema.substring(0, 100),
        mergedPreview: currentMergedString.substring(0, 100),
        schemasEqual: schema === currentMergedString
      });
      
      return actualChanges;
    } catch (err) {
      console.error('❌ VersionControls: Error checking for actual changes:', err);
      return isModified; // Fallback to basic isModified check
    }
  }, [schema, patches, isModified]);
