...
export const ParametersNode = memo(({ data, isConnectable, id, selected, onAddNotation, onToggleCollapse }: ParametersNodeProps) => {
  const { label, paramDetails = [], notations = [], notationCount = 0, hasNotations = false, hasMoreLevels = false, isCollapsed = false, path, hasCollapsibleContent } = data;

  // Determine if node has children
  const hasChildren = hasCollapsibleContent || hasMoreLevels || paramDetails.length > 0;
