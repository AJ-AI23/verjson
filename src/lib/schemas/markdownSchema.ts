...
/**
 * Get the type for a property within a markdown array item (page or embed)
 */
export function getMarkdownItemPropertyType(arrayPath: string, propertyName: string): string | null {
  const itemSchema = getMarkdownArrayItemSchema(arrayPath);
