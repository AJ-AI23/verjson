...
// Helper to apply a single JSON patch operation
function applyPatchOperation(doc: any, op: any): any {
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'fetchDocument':
        const docId = requestBody.id;
        const targetVersionId = requestBody.document_version_id;
        
        logger.debug('Fetching document with versions applied', { docId, targetVersionId });
        
        if (!docId) {
          logger.warn('Missing document id parameter');
        ...
        try {
          // Fetch the document metadata (without content - we'll build it from versions)
          logger.logDatabaseQuery('documents', 'SELECT', { docId });
          ...
          logger.logDatabaseQuery('document_versions', 'SELECT', { docId });
          ...
          // If no versions exist, return document with empty content
          if (!versions || versions.length === 0) {
            logger.info('No versions found, returning empty content');
          ...
          // Filter versions up to target version ID if specified
          ...
          if (targetVersionId) {
            ...
            if (targetIndex === -1) {
              logger.warn('Target version not found', { targetVersionId });
              return new Response(JSON.stringify({ error: 'Target version not found' }), {
                status: 404,
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
              });
            }
            applicableVersions = versions.slice(0, targetIndex + 1);
            targetVersion = versions[targetIndex];
          } else {
            // Use the latest version info
            targetVersion = versions[versions.length - 1];
          }

          // Apply versions server-side
          // Find the latest released version as base
          let baseContent = {};
          let startIndex = 0;
          
          for (let i = applicableVersions.length - 1; i >= 0; i--) {
            const version = applicableVersions[i];
            if (version.is_released && version.is_selected) {
              if (version.full_document) {
                baseContent = JSON.parse(JSON.stringify(version.full_document));
                startIndex = i + 1;
                logger.debug('Found released version as base', { 
          ...
          for (let i = startIndex; i < applicableVersions.length; i++) {
            const version = applicableVersions[i];
            if (version.is_selected && version.patches && Array.isArray(version.patches) && version.patches.length > 0) {
              try {
                // Apply each patch operation
                for (const op of version.patches) {
                  content = applyPatchOperation(content, op);
                }
                logger.debug('Applied version patches', { 
                  version: `${version.version_major}.${version.version_minor}.${version.version_patch}`,
                  patchCount: version.patches.length 
                });
              } catch (patchError) {
                logger.error('Failed to apply patch', { version: version.id, error: patchError });
                // Continue with current content on error
              }
            }
          }

          logger.info('Successfully built document content', { 
            docId, 
            appliedVersions: applicableVersions.length,
            targetVersion: targetVersion ? `${targetVersion.version_major}.${targetVersion.version_minor}.${targetVersion.version_patch}` : null
          });
          ...
          return new Response(JSON.stringify({ 
            ...
            version: targetVersion ? {
              id: targetVersion.id,
              version_major: targetVersion.version_major,
              version_minor: targetVersion.version_minor,
              version_patch: targetVersion.version_patch,
              description: targetVersion.description,
              is_released: targetVersion.is_released,
          ...
          logger.error('Error fetching document with versions', error);
          return new Response(JSON.stringify({ error: error instanceof Error ? error.message : 'Failed to fetch document' }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
      ...
      default:
        ...
        return new Response(JSON.stringify({ error: 'Invalid action. Supported actions: listDocumentsByWorkspace, createDocument, updateDocument, deleteDocument, listSharedDocuments, fetchDocument' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
    }
